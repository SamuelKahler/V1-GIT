(function(){
  ...
  function rebuildViews(){
    state.byId=new Map(state.observations.map(row=>[row.sourceId,row]));state.displayPicks=canonicalRows(state.observations.filter(row=>['MLB_DATA','DAILY_IMPORT'].includes(row.source))).map(displayPick);state.evidenceObservations=buildEvidenceObservations();
  ...
  function gradeIdentity(row){return [row.date||row.pick_date||'',normalizeTeam(row.selectedTeam||row.selected_team)||'',normalizeTeam(row.opponent)||'',upper(row.market||''),upper(row.period||''),finite(row.line)??'',finite(row.odds)??''].join('|');}
  function canonicalRows(rows){const chosen=new Map();const priority=row=>row.source==='DAILY_IMPORT'?3:row.source==='MLB_DATA'?2:1;rows.forEach(row=>{const key=gradeIdentity(row);const current=chosen.get(key);if(!current||priority(row)>priority(current))chosen.set(key,row);});return [...chosen.values()];}
  function applyOneGrade(row,grade){const incoming=upper(grade.result);const preserve=row.authoritativeResult&&VALID_RESULTS.has(row.result);if(!preserve&&VALID_RESULTS.has(incoming)){row.result=incoming;row.status=incoming;}else if(!preserve&&['PENDING','UNVERIFIED'].includes(incoming)){row.result=null;row.status=incoming;}row.gamePk=grade.gamePk||row.gamePk;row.selectedTeam=grade.selectedTeam||grade.selected_team||row.selectedTeam;row.opponent=grade.opponent||row.opponent;row.gradeReason=grade.gradeReason||grade.grade_reason||row.gradeReason;row.metadataStatus=grade.gamePk?'RESOLVED':(grade.metadataStatus||row.metadataStatus);row.environment=grade.environment||row.environment;row.updatedAt=grade.updatedAt||grade.updated_at||new Date().toISOString();}
  function applyGrades(rows){if(!Array.isArray(rows))return{applied:0};let applied=0;rows.forEach(grade=>{const exact=state.byId.get(grade.pickId||grade.sourceId);const identity=gradeIdentity(grade);const targets=state.observations.filter(row=>row===exact||gradeIdentity(row)===identity);if(!targets.length)return;[...new Set(targets)].forEach(row=>{applyOneGrade(row,grade);applied++;});});rebuildViews();const cacheSaved=save();publish();return{applied,total:state.observations.length,cacheSaved};}
