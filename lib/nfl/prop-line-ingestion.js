...
async function getEventForGame(game){
  if(isHistorical(game.kickoffAt)){
    // NFL Historical Event Match Fix V1: Try multiple pregame snapshots
    // Odds API historical events endpoint requires searching across time windows
    const pregameWindows=[1440,720,360,180,60,30]; // 24h, 12h, 6h, 3h, 1h, 30m before kickoff
    let lastError=null;
    for(const minutes of pregameWindows){
      try{
        ...
        if(event){
          return {event,usage,historical:true,pregameWindow:minutes};
        }
        // Event not found in this window, continue to next window
      }catch(error){
        lastError=error;
        // Network error or rate limit, try next window
      }
    }
    // All pregame windows exhausted without match
    return {event:null,usage:null,historical:true,pregameWindow:null,searchError:lastError?.message||'No matching event in any pregame window'};
  }
...
export async function importNflRealPropLines({season=2025,week=null,markets=DEFAULT_MARKETS,dryRun=false}={}){
  ...
  for(const game of gameList){
    try{
      ...
      if(!eventResult.event){failures.push({game:`${game.awayTeam}@${game.homeTeam}`,reason:eventResult.searchError||'Odds API event not found',window:eventResult.pregameWindow});continue;}
      matchedEvents++;
