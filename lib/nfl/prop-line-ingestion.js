...
async function getEventForGame(game){
  if(isHistorical(game.kickoffAt)){
    // Historical event discovery is snapshot-based. An event can disappear from the
    // historical events list shortly before kickoff even though its event-odds
    // snapshot is still queryable. Search several pregame snapshots and keep the
    // first exact team match. These discovery calls have returned zero provider
    // usage in our production checks, but we still retain the latest usage headers.
    const discoveryMinutes=[24*60,12*60,6*60,3*60,60,30,10];
    let lastUsage=null;
    for(const minutes of discoveryMinutes){
      const date=isoBefore(game.kickoffAt,minutes);
      const {payload,usage}=await oddsFetch(`/v4/historical/sports/${SPORT}/events?date=${date}&dateFormat=iso`);
      lastUsage=usage||lastUsage;
      const event=findEvent(unwrap(payload),game);
      if(event) return {event,usage:lastUsage,historical:true,discoveredAt:date};
    }
    return {event:null,usage:lastUsage,historical:true,discoveredAt:null};
  }
...
export async function previewNflPropSmartBackfill({season=2025,startWeek=1,endWeek=18,markets=DEFAULT_MARKETS,maxEstimatedCredits=18000}={}){
  ...
  return {season:Number(season),startWeek:first,endWeek:last,markets:marketList,weeks,totalGames,totalHistoricalGames,estimatedHistoricalEventOddsCredits:totalEstimate,maxEstimatedCredits:Number(maxEstimatedCredits)||18000,withinConfiguredBudget:totalEstimate<=(Number(maxEstimatedCredits)||18000),note:'This is an upper-bound planning estimate. It does not verify that your provider plan includes historical player props.'};
}
...
export async function runNflPropSmartBackfill({season=2025,startWeek=1,endWeek=18,markets=DEFAULT_MARKETS,maxEstimatedCredits=18000}={}){
  ...
  if(!preview.withinConfiguredBudget) throw httpError(`Smart backfill blocked: estimated ${preview.estimatedHistoricalEventOddsCredits} credits exceeds configured budget ${preview.maxEstimatedCredits}.`,400,preview);
