...
export const PROMPT_RUNTIME_VERSION = 'F5_PROMPT_RUNTIME_V1';
...
export function enforcePromptRules({slate,aiPayload,weights}){
  const games=[];
  for(const game of slate.games){
    const ai=getAiGame(aiPayload?.games,game)||{};
    const choice=normalizeTeam(ai.selected_team);
    if(!choice || (choice!==game.away&&choice!==game.home)){
      games.push({...game,selectedTeam:null,decision:'NO_PLAY',qualificationReason:'AI selected No Play or returned no valid side.',categoryScores:null,finalModelScore:null,projectedF5Pct:null,edge:null,impliedPct:null,price:null,sources:ai.sources||[],rationales:ai.rationales||{}});continue;
    }
    const side=selectedSide(game,choice);
    if(!side){games.push({...game,selectedTeam:choice,decision:'NO_PLAY',qualificationReason:'No authoritative F5 price was supplied for the selected side.',categoryScores:null,finalModelScore:null,projectedF5Pct:null,edge:null,impliedPct:null,price:null,sources:ai.sources||[],rationales:ai.rationales||{}});continue;}
    ...
    else if(fms<6.75){decision='PASSED';reason=`Final Model Score ${fms.toFixed(2)} is below 6.75.`;}
    else if(favorite&&side.odds<-135){decision='PASSED';reason='Favorite is outside the preferred -135-or-better range.';}
    else if(edge<7.5){decision='PASSED';reason=`Projected edge ${edge.toFixed(1)} pp is below 7.5 pp.`;}
    games.push({
      ...game,selectedTeam:choice,selectedLine:side.line,price:side.odds,priceLabel:priceLabel(side.odds),decision,qualificationReason:reason,
      categoryScores:scores,finalModelScore:fms,projectedF5Pct:projected,impliedPct:implied,edge,
      ...
      bullishIndicators:bullish,lineupCertainty:clean(ai.lineup_certainty)||null,lineupRisk:clean(ai.material_lineup_risk)||null,rationales:ai.rationales||{},sources:Array.isArray(ai.sources)?ai.sources:[]
    });
  }
  const topPlays=games.filter(g=>g.decision==='TOP_PLAY').sort((a,b)=>b.edge-a.edge||b.finalModelScore-a.finalModelScore);
  const passed=games.filter(g=>g.decision!=='TOP_PLAY');
  return {games,topPlays,passed};
}
...
export async function runF5PromptRuntime({date,weights:inputWeights}={}){
  ...
  const enforced=enforcePromptRules({slate,aiPayload,weights});
