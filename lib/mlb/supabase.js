...
function configuration() {
    ...
    if (!url || !key) {
        throw createSupabaseError(
            "SUPABASE_URL and SUPABASE_SECRET_KEY (or SUPABASE_SERVICE_ROLE_KEY) must be configured in Vercel.",
            503
        );
    }
...
async function requestSupabase(
    ...
    if (finalError && !finalError.statusCode) {
        throw createSupabaseError(
            `Supabase network request failed after ${attempts} attempts.`,
            502,
            { endpoint: path, cause: finalError.message || String(finalError) }
        );
    }
