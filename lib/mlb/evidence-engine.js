...
function qualifyResult(variant, response, totalConditionCount) {
...
class SportsEdgeEvidenceEngine {
    ...
    async report(criteria = {}, options = {}) {
        const normalized = normalizeCriteria({ ...criteria, limit: criteria.limit || options.limit || 100 });
        ...
        for (const variant of variants) {
            ...
            responses.push(qualifyResult(variant, response, conditionCount));
        }
