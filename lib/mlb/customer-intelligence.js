...
function matrixCriteria(criteria = {}, period = 'FULL_GAME') {
  const normalized = {
    ...
    minimumHitRate: Number(criteria.minimumHitRate || 52.5)
  };
  delete normalized.limit;
  return normalized;
}

function normalizeMatrixTrend(row = {}, side = 'SELECTED', invertOutcome = false) {
  ...
  const rawWins = finite(row.wins, 0);
  const rawLosses = finite(row.losses, 0);
  const pushes = finite(row.pushes, 0);
  const wins = invertOutcome ? rawLosses : rawWins;
  const losses = invertOutcome ? rawWins : rawLosses;
  ...
  return {
    ...
    record: {
      ...
      losses,
      pushes,
      sampleSize: finite(row.sample_size, wins + losses + pushes),
      hitRate: decisions ? percent((wins / decisions) * 100) : null,
      roiPercent: null,
      profitUnits: null,
      roiSample: 0,
...
async function buildTrendSignals(criteria, period) {
  ...
  const signals = (Array.isArray(selected?.trends) ? selected.trends : [])
    .map((row) => normalizeMatrixTrend(row, 'SELECTED', false));
  ...
  if (opponentCriteria) {
    ...
    const market = String(criteria.market || '').toUpperCase();
    const invertOpponent = period === 'F5' || ['ML','MONEYLINE','SIDE'].includes(market);
    signals.push(...(Array.isArray(opponent?.trends) ? opponent.trends : [])
      .map((row) => normalizeMatrixTrend(row, 'OPPONENT', invertOpponent)));
  }
...
export async function buildCustomerIntelligence(criteria = {}, options = {}) {
  ...
  return {
    version: 'DECISION_ENGINE_V3',
    release: 'ACTIONABLE_EVIDENCE_DECISION_ENGINE_V3',
