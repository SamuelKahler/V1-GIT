...
export async function buildCustomerIntelligence(criteria = {}, options = {}) {
  const report = await sportsEdgeEvidenceEngine.report(criteria, {
    ...options,
    minimumSample: options.minimumSample || 1,
    maximumVariants: 1
  });
  ...
  const baselineCriteria = {
    teamAbbreviation: criteria.teamAbbreviation,
    dateFrom: criteria.dateFrom,
    dateTo: criteria.dateTo,
    limit: criteria.limit || options.limit || 200
  };
  if (period === 'F5' && criteria.f5Line !== undefined) baselineCriteria.f5Line = criteria.f5Line;
  const baselineResponse = await sportsEdgeQueryEngine.query(baselineCriteria);
  const baseline = sectionFromResult({
    name: period === 'F5' ? 'This-season F5 baseline' : 'This-season team baseline',
    criteria: baselineCriteria,
    removedConditions: [],
    matchedConditionCount: 1,
    totalConditionCount: 1,
    exactnessPercent: 100,
    summary: periodSummary(baselineResponse?.summary || {}, period),
    supportingGames: baselineResponse?.supportingGames || [],
    evidenceScore: 0
  }, period);

  return {
    version: 'CUSTOMER_INTELLIGENCE_V2',
    source: 'MLB_INTELLIGENCE_GAME_LOGS',
    period,
    criteria: report.criteria,
    headline: {
      hitRate: exact.record.hitRate,
      record: exact.record,
      evidenceScore: exact.evidenceScore,
      exactnessPercent: 100,
      label: exact.name
    },
    ...
    disclosure: {
      ...
      roi: exact.record.roiPercent === null
        ? 'ROI is unavailable because verified historical odds are missing for this sample.'
        : `ROI uses ${exact.record.roiSample} historical games with verified odds.`
    },
