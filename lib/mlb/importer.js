...
import { rebuildEnvironments } from "./environment-engine.js";
...
export async function importDateRange({
    ...
    try {
        ...
        if (!normalizedDryRun && successfulGamePks.length > 0) {
            try {
                environmentRebuild = await rebuildEnvironments({
                    startDate,
                    endDate
                });
            } catch (error) {
