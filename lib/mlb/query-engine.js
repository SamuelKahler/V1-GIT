...
function normalizeCriteria(criteria = {}) {
    ...
    const normalized = {
        gamePk: optionalInteger(criteria.gamePk, "gamePk", { minimum: 1 }),
        ...
        opponentAbbreviation: optionalText(criteria.opponentAbbreviation, { upper: true }),
        ...
        f5Line: optionalNumber(criteria.f5Line, "f5Line", { minimum: -2, maximum: 2 }),
        totalLine: optionalNumber(criteria.totalLine, "totalLine", { minimum: 0, maximum: 30 }),
        totalDirection: optionalText(criteria.totalDirection, { upper: true }),
        market: optionalText(criteria.market, { upper: true }),
        period: optionalText(criteria.period, { upper: true }),
        gamesInSeries: optionalInteger(criteria.gamesInSeries, "gamesInSeries", { minimum: 1, maximum: 7 }),
        restDays: optionalInteger(criteria.restDays, "restDays", { minimum: 0, maximum: 30 }),
        previousRunsScored: optionalInteger(criteria.previousRunsScored, "previousRunsScored", { minimum: 0 }),
        previousRunsAllowed: optionalInteger(criteria.previousRunsAllowed, "previousRunsAllowed", { minimum: 0 }),
        sweep: optionalBoolean(criteria.sweep, "sweep"),
        avoidSweep: optionalBoolean(criteria.avoidSweep, "avoidSweep"),
        minimumSample: optionalInteger(criteria.minimumSample, "minimumSample", { minimum: 1, maximum: 500 }),
...
class SportsEdgeQueryEngine {
    ...
    async trendMatrix(criteria = {}) {
        const normalized = normalizeCriteria(criteria);
        return callRpc("sports_edge_mlb_trend_matrix", { p_criteria: normalized });
    }
}
