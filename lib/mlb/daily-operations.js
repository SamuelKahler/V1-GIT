...
export async function loadCanonicalDailyPicks() {
  ...
  const stored = await loadAdminStoredPicks();
  if (!stored.length) return legacy;
  const storedByDate = groupPicksByDate(stored);
  const mergedLegacy = legacy.filter(pick => !storedByDate.has(pick.date));
  return [...stored, ...mergedLegacy].sort((a, b) => b.date.localeCompare(a.date) || a.sourceIndex - b.sourceIndex);
}
...
export async function canonicalDailyImportText() {
  ...
  const storedDates = new Set(stored.map(pick => pick.date));
  const legacyByDate = groupPicksByDate(legacy.filter(pick => !storedDates.has(pick.date)));
  const storedByDate = groupPicksByDate(stored);
  const allDates = [...new Set([...storedByDate.keys(), ...legacyByDate.keys()])].sort().reverse();
  const lines = [];
  for (const date of allDates) {
    const [year, month, day] = date.split('-');
    lines.push(`${Number(month)}/${Number(day)}/${year}`);
    const group = storedByDate.get(date) || legacyByDate.get(date) || [];
