...
-- Makes first-five evidence line-aware and keeps market results isolated.
-- Safe to run after migration 005.
...
with parameters as (
  ...
  select
    ...
    case when not g.f5_available then null when g.f5_away_score > g.f5_home_score then 'WIN' when g.f5_away_score < g.f5_home_score then 'LOSS' else 'PUSH' end as f5_result_raw,
  ...
  select
    ...
    case when not g.f5_available then null when g.f5_home_score > g.f5_away_score then 'WIN' when g.f5_home_score < g.f5_away_score then 'LOSS' else 'PUSH' end as f5_result_raw,
  ...
  select p.*,
         ...
         case
           when not p.f5_available then null
           when x.f5_line is null or x.f5_line = 0 then p.f5_result_raw
           when (p.team_f5_score + x.f5_line) > p.opponent_f5_score then 'WIN'
           when (p.team_f5_score + x.f5_line) < p.opponent_f5_score then 'LOSS'
           else 'PUSH'
         end as f5_result
