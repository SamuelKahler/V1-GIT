-- Sports Edge MLB Decision Engine V2
-- Ranked evidence hierarchy:
--   1. Exact trend-category + environment matches
--   2. Applicable trend-category matches
--   3. Applicable environment matches
...
create or replace function public.sports_edge_mlb_decision_engine_v2_audit()
returns jsonb
language sql
security definer
set search_path = public, mlb, ops
...
select jsonb_build_object(
  ...
  'evidenceLevels',jsonb_build_array('EXACT','TREND','ENVIRONMENT'),
...
revoke all on function public.sports_edge_mlb_decision_engine_v2_audit() from public;
grant execute on function public.sports_edge_mlb_decision_engine_v2_audit() to service_role;
